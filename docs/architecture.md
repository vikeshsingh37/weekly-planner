...
A conversational AI agent that helps users plan their week through natural language. The agent collects tasks, schedules them deterministically, and updates the plan interactively across a session.

**Core capabilities:**
- Parse tasks (name, duration, priority, deadline, optional fixed start time) from freeform text
- Schedule tasks using Earliest-Deadline-First into available work-hour slots
- Pin tasks to exact times at creation — no extra round-trip needed
- Automatically split long tasks into focus blocks (configurable max, default 90 min) with breaks between them
...
- Remove tasks with confirmation before deletion, then compact the schedule
- Fetch weather forecasts (Open-Meteo) to inform outdoor activity scheduling
- Persist session state (tasks, preferences, conversation history) — Postgres by default, JSON files as fallback
- **Multi-user auth** — JWT-based login/register; each user's data is fully isolated, scoped by user ID derived from their email
- Web UI: login screen, then a two-panel layout with real-time chat on the left and a live week-view calendar + settings on the right
...
| Overhead at 100k users | Graph serialization, node checkpointing | Stateless HTTP handler + Postgres session |
...
- `impl/` provides one concrete implementation of each: `EDFScheduler`, `JSONSessionManager` / `PostgresSessionManager`, `ToolRunner`, plus `auth` (user management) and `weather` (Open-Meteo integration).

The agent depends only on `api/`. Swapping Postgres sessions for Redis, or EDF for an ILP-based scheduler, is a one-file change with no edits to `agent/agent.py`. This is also what makes the eval suite trivial — each case runs against a fresh in-memory `JSONSessionManager(session_file=None)` with no I/O.
...
All other API endpoints require `Authorization: Bearer <token>`. The dependency `_require_user` extracts and verifies the token, returning the `user_id` that scopes every database operation.
...
This string keys the Postgres row in `planner_sessions` (or the session directory in fallback JSON mode). Different users never share state, so isolation is enforced at the storage level with no additional access-control logic.
...
### `parse_and_add_tasks`
Accepts an array of structured task data the AI agent extracts from natural language. The agent handles NLP (duration inference, priority estimation); this tool writes to session state.

Key field: **`start_time` (HH:MM, optional)** — when set, the task is immediately pinned to that slot (`scheduled_start`, `scheduled_end`, `pinned=True`) at creation time. This eliminates the need for a separate `move_task` call when the user specifies an exact time. `schedule_tasks` then packs remaining unpinned tasks around these anchors.

Separating LLM extraction from state mutation means an extraction error doesn't corrupt the schedule — the tool either accepts valid data or returns an error.
...
### `move_task`
Pins a task to a user-specified time slot, marks it `pinned=True`, then calls `schedule_tasks` to fill the remaining time around it. Pinned tasks are never moved or split by the auto-scheduler. Used when the user wants to reposition an *existing* task — for new tasks with a known time, `start_time` in `parse_and_add_tasks` is preferred.

### `remove_task`
Removes a task by ID (or name as fallback). Does **not** save or reschedule — the agent always calls `schedule_tasks` after all removals are complete, which is when the single save occurs. The agent asks for user confirmation before issuing any `remove_task` call.
...
- `timezone`: IANA timezone (e.g. `America/New_York`, `Asia/Kolkata`). Validated via `zoneinfo.ZoneInfo`.
...
EDF is provably optimal for single-machine scheduling when all tasks must complete before their deadline and preemption isn't allowed — which matches the weekly planning problem exactly.
...
**Conflict detection** (`check_conflicts`) is chunk-aware: for split tasks it inspects each individual block's interval rather than the coarse `scheduled_start–scheduled_end` span.

**Break handling:** `break_minutes` serves double duty — gap between different tasks *and* gap between consecutive blocks of the same split task.
...
`Task.chunks` is persisted so the REST `/api/schedule` endpoint can serve chunk data to the calendar without re-running the scheduler.

### Session manager hierarchy

```
AbstractSessionManager   (api/memory.py)
    └── _BaseSessionManager   (impl/memory.py)
            ├── JSONSessionManager      — file-backed fallback
            └── PostgresSessionManager  — JSONB column in planner_sessions table
```

`_BaseSessionManager` holds all in-memory state and a `threading.Lock`. All task mutations (`add_task`, `remove_task_by_id`, `replace_tasks`, etc.) acquire the lock before modifying the list, making them safe for concurrent tool execution. Subclasses only override `save()`, `reload()`, and `_load()`.

### Storage backend selection

`server.py` checks for `DATABASE_URL` at startup:
- **Set** → `PostgresSessionManager` — state stored as JSONB in `planner_sessions` on the same Postgres instance used by Langfuse. Table is created automatically on first server start.
- **Unset** → `JSONSessionManager` — state stored in `sessions/{user_id}/state.json`.

### Parallel tool execution and thread safety

When Claude returns multiple tool calls in one response (e.g. three `remove_task` calls), the agent runs them concurrently via `ThreadPoolExecutor`. Each tool call that mutates session state acquires `_BaseSessionManager._lock` for the duration of the list operation — so concurrent removals chain correctly rather than racing:

```
Thread 1: acquires lock → removes task B from [A,B,C] → releases lock
Thread 2: acquires lock → removes task C from [A,C]   → releases lock
```

`remove_task` deliberately does **not** call `save()` — the single save happens in `schedule_tasks` at the end of the removal flow. This avoids N serialized file/DB writes for a bulk delete.

### Why store the full conversation history?

The model's context window is the memory. No vector DB, no summarisation pipeline, no embedding. The session state stores the history in the format the API expects — multi-turn context is free. The cost: very long sessions eventually hit the token limit.

### Preference sync across WebSocket and REST

The WebSocket handler holds a session manager in memory for the duration of the connection. The REST `POST /api/preferences` endpoint creates a separate instance, writes to the DB, and returns. On the agent's next `chat()` call, `session.reload()` re-reads state from the DB, picking up the new preferences before any tool call runs.

---

## 7. Agent Tool-Call Strategy (System Prompt)

The system prompt encodes explicit tool-call patterns to minimise round-trips:

| Scenario | Tool call sequence |
|----------|--------------------|
...
| Remove tasks | confirm with user → `get_schedule` (IDs) → `remove_task` × N in parallel → `schedule_tasks` → `get_schedule` |
| Move existing task | `get_schedule` (ID) → `move_task` → `get_schedule` |

Key rules enforced by the prompt:
- All new tasks from one request go in a **single** `parse_and_add_tasks` call (array input).
- Never call `schedule_tasks` between individual removals — one call at the end after all removes.
- Always ask for confirmation before any `remove_task`.
- Always use task UUID (not name) for `move_task` / `remove_task` — duplicate names are common.

---

## 8. Web UI and Event Streaming
...
*Calendar:*
- Vertical day-view timeline spanning **midnight to midnight (24 hours)**; `PX_PER_MIN = 1.5` (90 px/hour). Auto-scrolls to `work_start` on each render so the user lands at business hours by default but can freely scroll to any time of day.
...
- A red dot + horizontal line marks the current time when viewing today's plan; the indicator is visible at any hour within the 24-hour range
- Fetched via `GET /api/schedule` on WebSocket connect and after every agent response

*Settings:*
- Settings gear icon lives in the **calendar header, next to the timezone label** (not the top navigation bar)
...
*Debug trace:*
- After each agent reply that involved at least one tool call, a `▶ Debug trace` button appears below the chat bubble
- Clicking it expands an inline panel listing every tool step executed during that turn, with its label and wall-clock execution time in milliseconds
- Replies with no tool calls (pure text responses) show no debug button
- The step log is captured from `stepLog` on the client at the moment the `response` event arrives, before the thinking bubble collapses

---

## 9. Production Considerations (100k+ Users)
...
**Focus block fragmentation.** At high task density, many small free slots cause a long task's blocks to be scattered across the day. Fix: add a "minimum block size" threshold — don't start a chunk unless the remaining slot is at least N minutes.
...
3. **Token revocation** — add a `jti` claim and a blocklist for sign-out
4. **User store** — move `data/users.json` to a Postgres table
...
| Feature | Why Not |
|---------|---------|
| Calendar API (Google Calendar, Outlook) | Requires OAuth per user, adds infra complexity |
| Long-term memory across weeks | Out of scope — single-session planner |
| Vector memory / semantic search | Overkill for <50 tasks per session |
| Constraint solver (ILP) | EDF is optimal for the single-machine case; ILP adds exponential complexity with no practical benefit |
| DST-aware time arithmetic | All stored times are `HH:MM` wall-clock strings; DST transitions within a workday are an edge case |
...
| Per-task focus block override | All tasks share `Preferences.max_chunk_minutes` |
| Token revocation on sign-out | JWTs are stateless; sign-out is client-side only. Add a server-side blocklist for production |
| Password reset / email verification | No email service integrated |

---

## 11. Where the Agent Breaks

### Ambiguous task duration
*"I need to prepare for the meeting"* — the agent estimates a duration. The system prompt instructs asking when duration is absent; the agent sometimes proceeds with an estimate anyway.

### Focus blocks scattered by pinned tasks
If the user pins several tasks, the remaining free slots may be small and non-contiguous. A long task's focus blocks get placed in whatever fragments are available. Fix: add a "minimum viable block" threshold.

### Greedy chunk placement ignores other pending tasks
`_place_chunked()` consumes free slots greedily. If a high-priority long task is processed first (as EDF dictates), it may consume slots that a later short task with an earlier deadline could have used. Fix: consider partial placement and backtracking.

### Priority ties with no deadline
Two tasks with the same priority and no deadline are scheduled in insertion order (Python sort is stable). Fix: expose a secondary sort key or allow manual rank within a priority level.
...
## 12. What It Would Take to Fix the Major Gaps
...
| User store concurrency | SQLite WAL or Postgres users table | Low (mostly infra) |
